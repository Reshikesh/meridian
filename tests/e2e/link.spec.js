...
/* Decisions 32-37: the linked workbook.
   ...
   besides (DECISION-LOG 262). So the pickers are replaced before the app
   loads, and what they return is a REAL FileSystemFileHandle from the
   origin-private file system: it structured-clones into IndexedDB, survives a
   reload, and writes through `createWritable` exactly like a picked one. Only
   the two things the spike showed cannot be automated — the permission answer
   and a write that fails — are simulated, by patching the prototype. */
...
/* The stub, installed before any app script runs. `window.__link` is the test's
   hand on the permission and on the next write; `window.__file` is its hand on
   the bytes.

   The file lives in localStorage rather than in the origin-private file system:
   OPFS is refused outright on a `file://` origin ("certain files are unsafe for
   access within a Web application"), which is the same wall decision 32 is
   working around. IndexedDB is not refused, and the app really does store its
   handle there — so `put` and `get` are patched to carry a marker across the
   clone and hand back a live double on the way out. Both patches are the test's
   own; nothing in the app knows it is being watched.

   `lastModified` is a counter, not a clock: these tests freeze time, and a
   frozen clock would leave every write looking simultaneous. */
async function installPickers(page, opts) {
  await page.addInitScript(([name, options]) => {
    /* An init script runs again on every navigation, so the knobs live in
       localStorage: a test that sets `permission` and then reloads is testing
       a new browser session, not a fresh stub. */
    ...
    state.asks = 0;
    const persist = () => { try { localStorage.setItem(KNOBS, JSON.stringify(state)); } catch (e) { /* private mode */ } };
    persist();
    ...
    /* A double with methods cannot be structured-cloned, so what goes into
       IndexedDB is a marker and what comes out is a fresh double. */
    const realPut = IDBObjectStore.prototype.put;
    IDBObjectStore.prototype.put = function (value, key) {
      return realPut.call(this, value && value.__meridianFake ? { __meridianFake: true, name } : value, key);
    };
...
async function open(page, opts) {
  ...
  await installPickers(page, opts || {});
