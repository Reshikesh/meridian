...
Status: **all resolved on 30 Aug 2026.** These override `MERIDIAN-SPEC-v1.1.md` §13 wherever they differ. Claude Code does not reopen them; anything not covered here and not purely technical gets asked once, at the start of a phase. **Amended 5 Sep 2026 — 32–35 added (linked workbook).** **Amended 6 Sep 2026 — 34 and 35 rewritten on the 8a spike's measurements; 36 added (Protected View).** **Amended 7 Sep 2026 — 37 added (the three moments the app and the file disagree).** **Amended 9 Sep 2026 — 37 A qualified: the prompt is asked only when Meridian itself holds something to lose.** **Amended 9 Sep 2026 — 28: the version is shown under the header wordmark, on the owner's instruction.**
...
| 28 | Where the version shows | **One constant, four surfaces.** `src/core/version.js` is the single source, loaded first; the workbook stamps it into every export's `Meta.app_version`. It shows as `MERIDIAN 1.1.0` in the caps-label style in the Data sheet footer and below the three first-run choices, as the bare number under the header wordmark in smaller dim type, and as `title="Meridian 1.1.0"` on the wordmark itself. **Amended 9 Sep 2026 by the owner:** the header line replaces "nothing else in the header changes" — a build that can be read off a screenshot without hovering anything. It is stacked under the wordmark and costs no header height, because the theme toggle beside it is taller than both lines together. An allowed deviation from the mockup — added to the list QUALITY-BAR §1 names. |
...
| 37 | The three moments the app and the file disagree | **A — Link workbook, and the picked file already holds a dataset:** the decision-15 prompt appears before anything is written. Replace local reads the file in; Keep local overwrites the file on the next write. **Amended 9 Sep 2026 — and only when Meridian holds a dataset too.** With nothing in the app there is nothing to choose between, so the workbook is simply opened, which is B's own reasoning applied one moment earlier. Unqualified, this rule put *Keep local, overwrite the workbook* in the brand colour under Enter on the first-run screen, one keystroke from writing an empty dataset over the friend's only copy; the owner met it on a real reopen. **B — reconnect at the start of a session, file newer than the last mirror:** if the unexported counter is 0 the workbook holds everything the app had plus the edit, so it is adopted **silently** — the import report appears only if rows were rejected (decision 5). If the counter is above 0, both sides have changed and the prompt appears. **C — mid-session, the file changed under a write:** if the file cannot be read there is nothing to preserve, so the app writes and says so; if it can be read, **nothing is written** — EDITED OUTSIDE, and the click asks (34). Never merge. |
